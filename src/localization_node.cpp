...
void LocalizationNode::odomCallback(const Odometry::SharedPtr odom) {
  ...
  if (!has_last_odom_time_) {
    ...
    for(int i=0;i<3;++i)
      for(int j=0;j<3;++j)
        R(i,j) = Rm[i][j];
    Eigen::Vector3d v{ twist.linear.x,
                       twist.linear.y,
                       twist.linear.z };
    auto dp = R * v * dt;
  ...
  if (odom_history_.size()>1000) odom_history_.pop_front();
  processPendingTFs();
}
...
void LocalizationNode::processPendingTFs() {
  while (!tf_queue_.empty()) {
    ...
    rclcpp::Time t(ts.header.stamp);
    if (odom_history_.empty() || lidar_history_.empty()) break;
    if (odom_history_.back().first < t ||
        lidar_history_.back().first < t)
      break;
    processOneTF(ts);
    tf_queue_.pop_front();
  }
...
void LocalizationNode::processOneTF(const TransformStamped & ts) {
  ...
  if (!odom_opt || !lidar_opt) {
    RCLCPP_WARN(get_logger(),
      "Missing buffered odom or lidar ≤ TF, skipping");
    return;
  }
  ...
  RCLCPP_INFO(get_logger(),
    ...
    "  GT   pos:(%6.3f,%6.3f)",
    frame_count_++,
    ts.header.stamp.sec, ts.header.stamp.nanosec,
    o_s, o_ns, l_s, l_ns,
...
LocalizationNode::findLastBefore(const BufferT & buf, const rclcpp::Time & t)
{
  ...
  auto it = std::lower_bound(
    buf.begin(), buf.end(), t,
    [](const PairT & a, const rclcpp::Time & ts){
      return a.first < ts;
    });
