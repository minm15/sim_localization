...
LinK3D_Extractor::LinK3D_Extractor(int nScans, float minimumRange, float XYDisTh, float xDisTh, float yDisTh,
                                   int ptNumTh, int scanNumTh, int scoreTh)
    : mNScans(nScans), mMinimumRange(minimumRange), mXYDisTh(XYDisTh), mXDisTh(xDisTh), mYDisTh(yDisTh),
      mPtNumTh(ptNumTh), mScanNumTh(scanNumTh), mScoreTh(scoreTh) {}

void LinK3D_Extractor::removeClosedPoint(pcl::PointCloud<pcl::PointXYZ>& cloudIn,
    ...
    for (size_t i = 0; i < cloudIn.points.size(); ++i) {
        if (cloudIn.points[i].x * cloudIn.points[i].x + cloudIn.points[i].y * cloudIn.points[i].y +
...
void LinK3D_Extractor::computeCluster(const MatPt& areaCloud, MatPt& clustered) {
    ...
    for (int i = 0; i < areaSize; i++) {
        ...
        for (int j = 1; j < ptCnt; j++) {
            ...
            for (int k = 0; k < clusterCnt; k++) {
                ...
                if (abs(distXY(tmpPt) - means) < mXYDisTh && abs(xyMeans.first - tmpPt.x) < mXDisTh &&
                    abs(xyMeans.second - tmpPt.y) < mYDisTh) {
    ...
    for (int i = 0; i < clusterCnt; i++) {
        ...
        for (int j = 1; j < clusterCnt; j++) {
            ...
            if (abs(means1 - means2) < 0.6 && abs(xyMeans1.first - xyMeans2.first) < 0.8 &&
                abs(xyMeans1.second - xyMeans2.second) < 0.8) {
...
void LinK3D_Extractor::getDescriptor(const std::vector<pcl::PointXYZ>& keyPoints, cv::Mat& descriptors) {
    ...
    for (size_t i = 0; i < keyPoints.size(); i++) {
        ...
        for (int indNum = 0; indNum < 3; indNum++) {
            ...
            // Each dimension of the descriptor corresponds to the distance between the current keypoint and the closest
            // keypoint in the corresponding sector area.
...
void LinK3D_Extractor::operator()(pcl::PointCloud<pcl::PointXYZ>& cloudIn, std::vector<pcl::PointXYZ>& keyPoints,
                                  cv::Mat& descriptors, std::vector<int>& index, MatPt& clustered) {
    if (cloudIn.empty()) {
        return;
    }
...
void LinK3D_Extractor::matchEdgePoints(MatPt& filtered1, MatPt& filtered2, vector<int>& index1, vector<int>& index2,
                                       MatPt& matchedPoint, vector<pair<int, int>>& vMatched) {
    if (vMatched.empty()) {
        return;
    }
